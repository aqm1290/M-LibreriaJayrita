...
class BuscadorProductos extends Component
{
    ...
    // Esta función mágica evita el error "Class Producto not found" en la vista
    public function getProductoDetalleProperty()
    {
        if (!$this->productoSeleccionado) {
            return null;
        }
        return Producto::with(['marca', 'categoria'])->find($this->productoSeleccionado);
    }
    ...
    {
        $this->reset([
            'nombre', 'codigo', 'marca_id', 'categoria_id',
            'stock_min', 'stock_max', 'precio_min', 'precio_max'
        ]);
