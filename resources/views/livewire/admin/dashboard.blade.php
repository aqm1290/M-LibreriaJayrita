<div class="space-y-12">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
        ...
        @if (auth()->user()->esCajero() || auth()->user()->esAdmin())
            @if ($cajaAbierta)
                <div
                    class="inline-flex items-center gap-5 px-10 py-6 bg-green-100 text-green-800 rounded-3xl font-black text-2xl shadow-2xl border-4 border-green-200">
    ...
    <div>
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-3xl font-black text-gray-900 flex items-center gap-4">
                <span
                    class="bg-yellow-400 text-white w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-lg">1</span>
    ...
    <div>
        <h2 class="text-3xl font-black text-gray-900 mb-8 flex items-center gap-4">
            <span
                class="bg-red-500 text-white w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-lg">2</span>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <!-- STOCK BAJO -->
            <div
                class="bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-3xl shadow-2xl border-4 border-yellow-300 p-8">
                <h3 class="text-2xl font-black text-yellow-800 mb-6">
                    Poca Existencia (1-10 und)
                </h3>
                <div class="space-y-4 max-h-96 overflow-y-auto">
                    @forelse($stockBajo as $p)
                        <div
                            class="bg-white p-5 rounded-2xl shadow-lg border-2 border-yellow-400 flex justify-between items-center">
                            <div>
                    ...
                    @empty
                        <p class="text-center text-yellow-700 text-xl py-12">¡Todo bien! No hay productos con poco stock
                        </p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-3xl shadow-2xl border-4 border-red-400 p-8">
                <h3 class="text-2xl font-black text-red-800 mb-6">
                    ¡SIN STOCK!
                </h3>
                <div class="space-y-4 max-h-96 overflow-y-auto">
                    @forelse($sinStock as $p)
                        <div
                            class="bg-white p-5 rounded-2xl shadow-lg border-2 border-red-500 flex justify-between items-center">
                            <div>
                                <p class="font-black text-xl">{{ $p->nombre }}</p>
                                <p class="text-gray-600">{{ $p->marca?->nombre }} • {{ $p->modelo?->nombre }}</p>
                            </div>
    ...
    <div>
        <h2 class="text-3xl font-black text-gray-900 mb-8 flex items-center gap-4">
            <span
                class="bg-gray-600 text-white w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-lg">3</span>
                ...
                @forelse($productosMuertos as $p)
                    <div class="bg-gray-50 p-6 rounded-2xl border-2 border-gray-300 hover:border-gray-500 transition">
                        ...
                        <div class="mt-4 flex justify-between items-center">
                            ...
                            <span
                                class="bg-red-100 text-red-800 px-4 py-2 rounded-full text-sm font-bold">Estancado</span>
                        </div>
                    </div>
                @empty
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-12">
        <div class="bg-white rounded-3xl shadow-2xl p-8">
            <h2 class="text-3xl font-black text-gray-900 mb-8 flex items-center gap-4">
                <span
                    class="bg-yellow-400 text-white w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-lg">4</span>
        ...
        <div class="bg-white rounded-3xl shadow-2xl p-8">
            <h2 class="text-3xl font-black text-gray-900 mb-8 flex items-center gap-4">
                <span
                    class="bg-yellow-400 text-white w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-lg">5</span>
                Top 10 Productos del Día
            </h2>
            <div class="space-y-5 max-h-96 overflow-y-auto">
                @forelse($topProductos as $i => $item)
                    <div
                        class="flex items-center justify-between p-6 rounded-2xl hover:bg-yellow-50 transition border-2 border-gray-100">
                        <div class="flex items-center gap-5">
                            <div
                                class="w-16 h-16 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-2xl flex items-center justify-center text-white font-black text-3xl shadow-xl">
                                {{ $i + 1 }}
                            </div>
                            <div>
                                <p class="font-black text-xl">{{ Str::limit($item->producto->nombre, 30) }}</p>
                                <p class="text-gray-600">{{ $item->producto->marca?->nombre }} •
                                    {{ $item->producto->modelo?->nombre }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="font-black text-3xl text-gray-900">{{ $item->total_vendido }}</p>
                            <p class="text-xl font-bold text-green-600">Bs
                                {{ number_format($item->total_ingresos, 2, ',', '.') }}</p>
                        </div>
                    </div>
                @empty
