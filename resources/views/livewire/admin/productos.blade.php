<div class="min-h-screen bg-gradient-to-br from-yellow-100 via-yellow-50 to-orange-100 py-10 px-6">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                ...
                <p class="mt-2 text-orange-700/90 text-base md:text-lg">
            ...
            <button wire:click="crear"
                class="inline-flex items-center justify-center gap-3 px-6 py-3 md:px-8 md:py-4
                       bg-gradient-to-r from-yellow-400 via-yellow-500 to-orange-500 text-white font-semibold md:font-bold
                       rounded-2xl shadow-[0_14px_40px_rgba(249,115,22,0.7)]
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="lg:col-span-2">
                    <input type="text" wire:model.live.debounce.500ms="search"
                        ...
                        class="w-full px-5 py-3.5 text-base border border-yellow-300 bg-white/80
                               rounded-2xl shadow-sm focus:border-yellow-500 focus:ring-2
                               focus:ring-yellow-200 outline-none transition">
                </div>

                <select wire:model.live="filtroCategoria"
                    class="px-5 py-3.5 border border-yellow-300 rounded-2xl bg-white/80 text-sm
            ...
            <div class="flex flex-wrap items-center gap-6">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" wire:model.live="soloStockBajo"
                        class="w-5 h-5 text-yellow-500 rounded focus:ring-yellow-200 border-yellow-300">
                    <span class="text-sm font-medium text-slate-800">Solo stock bajo (≤ 5)</span>
                </label>

                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" wire:model.live="mostrarInactivos"
                        class="w-5 h-5 border-yellow-300 rounded focus:ring-yellow-200">
                    <span class="text-sm font-medium text-slate-800">Mostrar inactivos</span>
                </label>

                <button wire:click="limpiarFiltros"
                    class="ml-auto px-5 py-2.5 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 font-medium rounded-xl border border-yellow-200 transition">
                    Limpiar filtros
                </button>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-yellow-200">
                    ...
                    <tbody class="divide-y divide-yellow-50">
                        @forelse($productos as $p)
                            <tr class="hover:bg-yellow-50/70 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="inline-flex items-center px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 text-sm font-semibold border border-emerald-200">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @if ($p->stock == 0)
                                        <span
                                            class="inline-flex items-center px-3 py-1.5 rounded-full bg-rose-600 text-white text-xs font-bold shadow-sm">
                                            Sin stock
                                        </span>
                                    @elseif($p->stock <= 5)
                                        <span
                                            class="inline-flex items-center px-3 py-1.5 rounded-full bg-orange-500 text-white text-xs font-bold ring-2 ring-orange-300/70 shadow-sm">
                                            {{ $p->stock }} und
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold border border-emerald-200">
                                            {{ $p->stock }} und
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <button wire:click="ver({{ $p->id }})"
                                            class="p-2.5 rounded-lg bg-yellow-50 hover:bg-yellow-100 border border-yellow-200 transition"
                                            title="Ver detalles">
                                        ...
                                        <button wire:click="editar({{ $p->id }})"
                                            class="p-2.5 rounded-lg bg-yellow-100 hover:bg-yellow-200 border border-yellow-200 text-yellow-800 transition"
                                        ...
                                        <button wire:click="confirmarEliminar({{ $p->id }})"
                                            class="p-2.5 rounded-lg {{ $p->activo ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700' }} text-white transition shadow-sm"
            ...
            <div class="lg:hidden">
                @forelse($productos as $p)
                    <div class="border-b border-yellow-50 p-6 hover:bg-yellow-50/60 transition last:border-0">
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="flex flex-wrap gap-2 mt-3">
                                    <span
                                        class="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-sm font-bold border border-emerald-200">
                                        Bs {{ number_format($p->precio, 2) }}
                                    </span>

                                    @if ($p->stock == 0)
                                        <span class="px-3 py-1 rounded-full bg-rose-600 text-white text-xs font-bold">
                                            Sin stock
                                        </span>
                                    @elseif($p->stock <= 5)
                                        <span
                                            class="px-3 py-1 rounded-full bg-orange-500 text-white text-xs font-bold">
                                            {{ $p->stock }} und
                                        </span>
                                    @else
                                        <span
                                            class="px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold">
                                            {{ $p->stock }} und
                                        </span>
                                    @endif
                                ...
                                <div class="flex gap-3 mt-4">
                                    <button wire:click="ver({{ $p->id }})"
                                        class="p-2.5 rounded-lg bg-yellow-50 hover:bg-yellow-100 border border-yellow-200 transition">
                                        <i data-lucide="eye" class="w-5 h-5 text-yellow-700"></i>
                                    </button>
                                    <button wire:click="editar({{ $p->id }})"
                                        class="p-2.5 rounded-lg bg-yellow-100 hover:bg-yellow-200 border border-yellow-200 transition">
                                        <i data-lucide="pencil" class="w-5 h-5 text-yellow-800"></i>
                                    </button>
                                    <button wire:click="confirmarEliminar({{ $p->id }})"
                                        class="p-2.5 rounded-lg {{ $p->activo ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700' }} text-white transition">
                                        <i data-lucide="{{ $p->activo ? 'x-circle' : 'check-circle' }}"
                                            class="w-5 h-5"></i>
                                    </button>
                                </div>
                            </div>
        ...
        <!-- PAGINACIÓN -->
        <div class="px-6 py-5 bg-white/90 border border-yellow-200 rounded-2xl shadow-sm">
    ...
    @if ($modal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div
                class="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] overflow-y-auto border border-yellow-200">
                <div
                    class="bg-gradient-to-r from-yellow-400 via-yellow-500 to-orange-500 text-white px-8 py-6 rounded-t-2xl flex items-center justify-between">
                    <div>
                        <h2 class="text-2xl md:text-3xl font-black tracking-tight">
                            {{ $productoId ? 'Editar producto' : 'Nuevo producto' }}
                        </h2>
                        <p class="mt-1 text-sm text-yellow-100/90">
                            Completa la información del producto.
                        </p>
                    </div>
                    <button wire:click="cerrarModal" class="text-yellow-100 hover:text-white text-xl leading-none">
                        ✕
                    </button>
                </div>
                ...
                <div class="p-8 space-y-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <select wire:model="categoria_id"
                                class="w-full px-4 py-2.5 border border-yellow-300 rounded-xl text-sm bg-white
                                       focus:outline-none focus:ring-2 focus:ring-yellow-200 focus:border-yellow-500">
                    ...
                    <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start.window="uploading = true; progress = 0"
                        ...
                        x-on:livewire-upload-progress.window="progress = $event.detail.progress" class="space-y-6">
                        <label class="block text-xs font-semibold text-slate-800 uppercase tracking-wide">
                            Imagen del producto
                        </label>
                        ...
                        <div class="mt-6 flex justify-center">
                            ...
                            @else
                                <div
                                    class="w-72 h-72 flex flex-col items-center justify-center rounded-3xl border-4 border-dashed border-yellow-300 bg-yellow-50/60">
                                    <svg class="w-24 h-24 text-yellow-300 mb-4" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                            d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
                                    <p class="text-orange-800 font-bold text-lg">Sin imagen</p>
                                    <p class="text-xs text-orange-600 mt-1">
                                        Sube una imagen del producto (máx. 5 MB)
                                    </p>
                                </div>
                            @endif
                        </div>
                ...
                <div class="flex justify-end gap-4 px-8 py-6 border-t border-yellow-200 bg-yellow-50/70 rounded-b-2xl">
                    <button wire:click="cerrarModal"
                        class="px-6 py-2.5 rounded-xl border border-yellow-300 bg-white text-slate-700 text-sm font-medium
                               hover:bg-yellow-50 hover:border-yellow-400 transition">
                        Cancelar
                    </button>
                    <button wire:click="guardar"
                        class="px-8 py-2.5 rounded-xl bg-gradient-to-r from-yellow-500 via-orange-500 to-orange-600 text-white text-sm font-semibold
    ...
    @if ($modalVer && $productoSeleccionado)
        <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-8">
            <div
                class="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-yellow-200">
                <div
                    class="bg-gradient-to-r from-yellow-400 via-yellow-500 to-orange-500 text-white p-8 rounded-t-2xl text-center">
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="text-center">
                        <img src="{{ $productoSeleccionado->imagen_url }}"
                            class="w-full max-h-80 object-cover rounded-2xl shadow-2xl border-8 border-white ring-4 ring-yellow-200">
                    </div>
                ...
                <div class="p-8 text-center border-t border-yellow-200 bg-yellow-50/70 rounded-b-2xl">
                    <button wire:click="cerrarModal"
                        class="px-10 py-3 bg-gradient-to-r from-yellow-500 via-orange-500 to-orange-600 hover:from-yellow-600 hover:via-orange-600 hover:to-orange-700
                               text-white font-semibold text-sm rounded-xl shadow-2xl transition">
                        Cerrar
                    </button>
                </div>
            </div>
