    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div>
                ...
                <p class="mt-2 text-base md:text-lg text-orange-800/90">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-yellow-200 p-5 md:p-6">
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                <input type="text" wire:model.debounce.500ms="search" placeholder="Buscar promoción o código..."
                    class="w-full md:flex-1 px-5 md:px-6 py-3.5 md:py-4 text-sm md:text-base
                           border border-yellow-200 rounded-2xl bg-white/80
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-yellow-200 overflow-hidden">
            ...
            <div class="hidden lg:block overflow-x-auto">
                            ...
                            <tr
                                ...
                                <td class="px-6 py-5 align-top">
                                    ...
                                    @if ($promo->descripcion)
                                        <p class="text-xs text-slate-600">
                                            {{ \Illuminate\Support\Str::limit($promo->descripcion, 80) }}</p>
                                    @endif
                                    @if ($promo->codigo)
                                        <span
                                            class="mt-1 inline-flex items-center px-3 py-1 text-[0.65rem] font-bold
                                                     text-orange-900 bg-yellow-200 rounded-full">
                                            {{ $promo->codigo }}
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-5 align-top">
                                    <span
                                        ...
                                        @elseif($promo->tipo === 'descuento_monto') bg-yellow-100 text-yellow-800
                                        @elseif($promo->tipo === '2x1') bg-blue-100 text-blue-800
                                        ...
                                        @switch($promo->tipo)
                                            ...
                                            @case('descuento_monto')
                                                - Bs {{ number_format($promo->valor_descuento, 2) }}
                                            @break
                                ...
                                <td class="px-6 py-5 align-top">
                                    @if (is_null($promo->limite_usos))
                                        <span
                                            class="inline-flex items-center px-4 py-1.5 rounded-full bg-slate-100 text-slate-800 font-semibold text-xs">
                                            Ilimitado
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-4 py-1.5 rounded-full bg-rose-100 text-rose-800 font-semibold text-xs">
                                ...
                                <td class="px-6 py-5 align-top text-xs text-slate-700">
                                    ...
                                    @else
                                        @if ($promo->categoria)
                                            <div>Categoría: {{ $promo->categoria->nombre }}</div>
                                        @endif
                                        @if ($promo->marca)
                                            <div>Marca: {{ $promo->marca->nombre }}</div>
                                        @endif
                                        @if ($promo->modelo)
                                ...
                                <td class="px-6 py-5 align-top">
                                    <div class="flex justify-end gap-2">
                                        <button wire:click="editar({{ $promo->id }})"
                                            class="p-2.5 rounded-lg bg-yellow-100 text-yellow-800 hover:bg-yellow-200 border border-yellow-200 shadow-sm transition"
                ...
                <div class="lg:hidden divide-y divide-yellow-50">
                    @forelse($promos as $promo)
                        <div class="{{ $promo->activa ? 'bg-white' : 'bg-rose-50/50' }} p-5">
                            <div class="flex flex-col gap-2">
                                <div class="flex items-center gap-2">
                                    <h3 class="font-semibold text-slate-900 text-base">{{ $promo->nombre }}</h3>
                                    @if (!$promo->activa)
                                        <span
                                            class="px-2.5 py-0.5 text-[0.65rem] font-bold text-rose-700 bg-rose-100 rounded-full">INACTIVA</span>
                                    @endif
                                </div>
                                <div class="flex flex-wrap items-center gap-2 text-xs">
                                    ...
                                    <span
                                        class="px-3 py-1 rounded-full font-bold {{ $promo->activa ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800' }}">
                                        {{ $promo->activa ? 'ACTIVA' : 'INACTIVA' }}
                                    </span>
                                </div>
                                ...
                                <div class="mt-3 flex gap-2">
                                    <button wire:click="editar({{ $promo->id }})"
                                        class="p-2.5 rounded-lg bg-yellow-100 text-yellow-800 hover:bg-yellow-200 border border-yellow-200 transition">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </button>
                                    <button wire:click="eliminar({{ $promo->id }})"
                ...
                @if ($modal)
                    <div
                        ...
                        <div
                            class="w-full max-w-4xl rounded-3xl bg-white/95 shadow-2xl border border-yellow-200 flex flex-col max-h-[95vh]">
                            <div
                                class="bg-gradient-to-r from-yellow-400 via-yellow-500 to-orange-500 text-white px-8 py-5 text-center rounded-t-3xl">
                ...
                'compra_lleva' => 'Compra X Lleva Y',
                                            ...
                                            {{ $tipo === $value
                                                ...
                                                : 'bg-white text-slate-700 border-yellow-200 hover:border-yellow-400' }}">
                                                {{ $label }}
                                            </button>
                                        @endforeach
                                    </div>
                                ...
                                @if (in_array($tipo, ['descuento_porcentaje', 'descuento_monto']))
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                        <div class="md:col-span-2">
                                            <label
                                                class="block text-xs font-semibold text-slate-800 mb-2 uppercase tracking-wide">
                                                Valor del descuento *
                                            </label>
                                        ...
                                        <div class="text-center text-2xl font-black text-orange-600">
                                ...
                                @if ($tipo === 'compra_lleva')
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div class="space-y-3">
                                            <label
                                                class="block text-xs font-semibold text-slate-800 mb-1 uppercase tracking-wide">
                                                Producto que se compra (X)
                                            </label>
                                            ...
                                            @if ($query_compra !== '' && !empty($resultados_compra))
                                                <div
                                                    class="mt-2 max-h-48 overflow-y-auto border border-yellow-200 rounded-2xl bg-white shadow-lg z-50">
                                                    @foreach ($resultados_compra as $res)
                                                        <button type="button"
                                                            wire:click="seleccionarCompra({{ $res['id'] }})"
                                                            class="w-full px-4 py-3 text-left flex justify-between items-center hover:bg-yellow-50 transition border-b last:border-0">
                                                            <div>
                                                                <div class="font-medium text-slate-900">{{ $res['nombre'] }}
                                                                </div>
                                                                <div class="text-xs text-slate-500">
                                                                    Código: {{ $res['codigo'] }} • Stock: {{ $res['stock'] }}
                                                                </div>
                                                            </div>
                                            ...
                                            <div class="mt-3">
                                                @if (count($products_compra))
                                                    @foreach ($products_compra as $p)
                                                        <span
                                                            class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold">
                                                            {{ $p['nombre'] }}
                                                        </span>
                                                    @endforeach
                                                @else
                                            ...
                                            @if ($query_regalo !== '' && !empty($resultados_regalo))
                                                <div
                                                    ...
                                                    @foreach ($resultados_regalo as $res)
                                                        <button type="button"
                                                            wire:click="seleccionarRegalo({{ $res['id'] }})"
                                                            class="w-full px-4 py-3 text-left flex justify-between items-center hover:bg-yellow-50 transition border-b last:border-0">
                                                            <div>
                                                                <div class="font-medium text-slate-900">{{ $res['nombre'] }}
                                                                </div>
                                                            ...
                                                            <span class="text-orange-600 text-xs font-bold">Agregar</span>
                                                        </button>
                                                    @endforeach
                                                </div>
                                            @endif
                                            ...
                                            <div class="mt-3">
                                                @if (count($products_regalo))
                                                    @foreach ($products_regalo as $p)
                                                        <span
                                                            class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-orange-100 text-orange-800 text-xs font-bold">
                                                            {{ $p['nombre'] }}
                                                        </span>
                                                    @endforeach
                                                @else
                                ...
                                <div class="bg-rose-50 border border-rose-100 rounded-2xl p-5 space-y-3">
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs md:text-sm">
                                        <label class="inline-flex items-center gap-2 cursor-pointer">
                                            <input type="radio" wire:model="limite_usos" value=""
                                                class="w-4 h-4 text-rose-600">
                                            Ilimitado
                                        </label>
                                        <label class="inline-flex items-center gap-2 cursor-pointer">
                                            <input type="radio" wire:model="limite_usos" value="1"
                                                class="w-4 h-4 text-rose-600">
                                            1 uso
                                        </label>
                                        <label class="inline-flex items-center gap-2 cursor-pointer">
                                            <input type="radio" wire:model="limite_usos" value="2"
                                                class="w-4 h-4 text-rose-600">
                                            2 usos
                                        </label>
                                        <label class="inline-flex items-center gap-2 cursor-pointer">
                                            <input type="radio" wire:model="limite_usos" value="3"
                                                class="w-4 h-4 text-rose-600">
                                            3 usos
                                        </label>
                                        <label class="inline-flex items-center gap-2 cursor-pointer">
                                            <input type="radio" wire:model="limite_usos" value="5"
                                                class="w-4 h-4 text-rose-600">
                                            5 usos
                                        </label>
                                    </div>
                                ...
                                @if ($tipo !== 'compra_lleva')
                                    <div class="space-y-4">
                                        <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                                            <input type="checkbox" wire:click="$toggle('aplica_todo')"
                                                @checked($aplica_todo)
                                        ...
                                        @if (!$aplica_todo)
                                            ...
                                            <div class="space-y-3">
                                                ...
                                                @if ($query !== '' && !empty($resultados))
                                                    <div
                                                        ...
                                                        @foreach ($resultados as $res)
                                                            <button type="button"
                                                                wire:click="agregarProducto({{ $res['id'] }})"
                                                                class="w-full px-4 py-3 text-left flex justify-between items-center hover:bg-yellow-50 transition border-b last:border-0">
                                                                <div>
                                                                    <div class="font-medium text-slate-900">
                                                                        {{ $res['nombre'] }}</div>
                                                                    <div class="text-xs text-slate-500">
                                                                        Código: {{ $res['codigo'] }} • Stock:
                                                                        {{ $res['stock'] }}
                                                                    </div>
                                                                </div>
                                                                <span class="text-emerald-600 text-xs font-bold">Agregar</span>
                                                ...
                                                <div class="mt-3">
                                                            ...
                                                            @foreach ($productosSeleccionados as $i => $p)
                                                                <span
                                                                    ...
                                                                    <button type="button"
                                                                        ...
                                                                        class="hover:bg-emerald-200 rounded-full w-5 h-5 flex items-center justify-center transition">
                                                                        ×
                                                                    </button>
                                                                </span>
                                                            @endforeach
                                                    ...
                                                    @else
                                                        <p class="text-xs text-slate-500 italic">Escribe para buscar
                                                            productos...</p>
                                                    @endif
                                                </div>
                                            </div>
                                ...
                                <div class="flex items-center gap-3">
                                    <input type="checkbox" wire:model="activa" id="activa"
                                        class="w-5 h-5 text-emerald-600 rounded border-yellow-300 focus:ring-emerald-500">
                                    <label for="activa" class="text-sm font-semibold text-slate-800 cursor-pointer">
                                        Promoción activa
                                    </label>
                                </div>
                            </div>
