<div class="min-h-screen bg-gradient-to-br from-amber-50 via-white to-amber-100 py-10 px-4 md:px-8">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <header class="text-center space-y-3">
            ...
            <h1
                class="text-3xl md:text-4xl lg:text-5xl font-black bg-gradient-to-r from-yellow-400 via-amber-500 to-orange-500 bg-clip-text text-transparent">
        ...
        {{-- PANEL DE FILTROS --}}
        <section
            class="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-amber-100 p-6 md:p-8 space-y-6">
            ...
            <div class="flex flex-col lg:flex-row gap-4 lg:items-end">
                ...
                <div class="w-full lg:w-72 space-y-1">
                    ...
                    <input type="text" wire:model.live.debounce.300ms="codigo" placeholder="Escanéalo o escríbelo"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 pt-2 border-t border-amber-100">
                ...
                <div class="space-y-1">
                    ...
                    <div class="flex gap-2">
                        <input type="number" wire:model.live="stock_min" placeholder="Mín."
                        ...
                        <input type="number" wire:model.live="stock_max" placeholder="Máx."
                            class="w-1/2 px-3 py-3 rounded-2xl border-2 border-amber-100 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 bg-amber-50/40 text-xs md:text-sm">
                    </div>
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <div class="flex gap-2">
                        <input type="number" step="0.01" wire:model.live="precio_min" placeholder="Desde"
                            class="w-1/2 px-3 py-3 rounded-2xl border-2 border-amber-100 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 bg-amber-50/40 text-xs md:text-sm">
                        <input type="number" step="0.01" wire:model.live="precio_max" placeholder="Hasta"
                            class="w-1/2 px-3 py-3 rounded-2xl border-2 border-amber-100 focus:border-amber-500 focus:ring-2 focus:ring-amber-200 bg-amber-50/40 text-xs md:text-sm">
                    </div>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-gradient-to-r from-yellow-400 to-amber-500 text-slate-900">
                        <tr>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-left text-[0.7rem] md:text-xs font-black">Foto
                            </th>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-left text-[0.7rem] md:text-xs font-black">Producto
                            </th>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-left text-[0.7rem] md:text-xs font-black">Código
                            </th>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-left text-[0.7rem] md:text-xs font-black">Marca /
                                categoría</th>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-center text-[0.7rem] md:text-xs font-black">Precio
                            </th>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-center text-[0.7rem] md:text-xs font-black">Stock
                            </th>
                            <th class="px-4 md:px-6 py-3 md:py-4 text-center text-[0.7rem] md:text-xs font-black">
                                Acciones</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($productos as $p)
                            <tr class="hover:bg-amber-50/70 transition-colors">
                                <td class="px-4 md:px-6 py-3 md:py-4">
                                    ...
                                    @else
                                        <div
                                            class="w-14 h-14 md:w-16 md:h-16 bg-slate-50 rounded-xl flex items-center justify-center border-2 border-dashed border-slate-200">
                                ...
                                <td class="px-4 md:px-6 py-3 md:py-4 space-y-1.5 md:space-y-2.5">
                                    @if (Route::currentRouteName() === 'caja.pos')
                                        <button type="button" wire:click="agregarAlCarrito({{ $p->id }})"
                                    ...
                                    <button type="button" wire:click="abrirProducto({{ $p->id }})"
    ...
    @if ($productoSeleccionado && $productoDetalle)
        <div class="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4"
            wire:click="cerrarModal">
            <div class="bg-white rounded-3xl shadow-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto" wire:click.stop>
                <div
                    class="bg-gradient-to-r from-amber-300 to-amber-400 px-6 md:px-10 py-6 rounded-t-3xl flex justify-between items-center">
                    <div>
                ...
                <div class="p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10">
                    <div class="text-center">
                        @if ($productoDetalle->url_imagen)
                            <img src="{{ asset('storage/' . $productoDetalle->url_imagen) }}"
                                class="max-w-full max-h-96 mx-auto rounded-2xl shadow-2xl bg-white">
                        @else
                            <div
                                class="bg-slate-50 border-4 border-dashed border-slate-200 rounded-2xl w-full h-72 md:h-96 flex items-center justify-center">
                    ...
                    <div class="space-y-4 md:space-y-6">
                        ...
                        @if (Route::currentRouteName() === 'caja.pos')
                            <button type="button" wire:click="agregarAlCarrito({{ $productoDetalle->id }})"
