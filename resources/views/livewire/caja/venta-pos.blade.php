    ...
    <div class="max-w-7xl mx-auto p-4 lg:p-6 grid gap-5 lg:grid-cols-[minmax(0,1.7fr)_minmax(0,1.3fr)]">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100"
                ...
                <div
                    class="hidden md:block bg-gradient-to-r from-[#FFE600] to-yellow-400 px-6 py-4 text-sm font-bold text-[#111] uppercase tracking-wide">
                ...
                <div class="divide-y divide-gray-100">
                    @forelse($productos as $p)
                        <div wire:key="prod-{{ $p->id }}"
                            ...
                            <div
                                class="hidden sm:flex items-center justify-center w-24 h-24 rounded-2xl bg-gray-50 border border-gray-200 overflow-hidden">
                                @if ($p->url_imagen)
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="mt-2 flex flex-wrap items-center gap-3 text-sm text-gray-600">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full bg-red-100 text-red-700 font-medium">
                                            Agotado
                                        </span>
                                    @endif
                                </div>
                    ...
                    @empty
                        @if ($search !== '')
                            <div class="py-24 text-center text-gray-400">
                                <p class="text-3xl font-light">No se encontraron productos para "{{ $search }}"
                                </p>
                            </div>
                        @endif
                    @endforelse
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-200">
                ...
                <div class="max-h-[320px] overflow-y-auto bg-gray-50">
                    @forelse($cart as $i => $item)
                        <div
                            class="px-6 py-5 border-b border-gray-200 hover:bg-white transition flex items-start gap-6">
                            <div class="flex-1 space-y-2">
                                ...
                                <div class="flex items-center gap-4 mt-4">
                                    ...
                                    <input type="text" inputmode="numeric" pattern="[0-9]*"
                                        ...
                                        wire:change="actualizarSubtotal({{ $i }})" onclick="this.select()"
                                        onwheel="this.blur()"
                ...
                <div class="p-6 space-y-6 bg-gradient-to-b from-white to-gray-50">
                    ...
                    <div>
                        <label
                            class="block text-sm font-bold text-gray-700 mb-3 uppercase tracking-wide">Cliente</label>
                            ...
                            <div class="mb-4 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
                                @foreach ($clientesEncontrados as $cli)
                                    <button type="button" wire:click="seleccionarCliente({{ $cli->id }})"
                                        ...
                                        <span
                                            class="bg-[#3483FA] text-white px-4 py-2 rounded-full text-sm font-bold shadow">Usar</span>
                                    </button>
                                @endforeach
                            </div>
                        @endif
                        ...
                        <div
                            class="flex justify-between items-center bg-gradient-to-r from-blue-50 to-indigo-50 p-5 rounded-2xl border-2 border-blue-200">
                            <div>
                    ...
                    @if ($cliente_id)
                        ...
                        @endphp
                        <div
                            class="bg-gradient-to-r from-amber-50 to-yellow-50 border-3 border-amber-400 rounded-2xl p-5 text-center shadow-xl">
                            <p class="text-xl font-black text-amber-800 flex items-center justify-center gap-3">
                                <svg class="w-9 h-9 text-amber-600" fill="currentColor" viewBox="0 0 20 20">
                                    <path
                                        d="M10 2l3.09 6.26L19 8.27l-4.5 4.36 1.06 6.18L10 15.82l-5.56 2.92 1.06-6.18L1 8.27l5.91-.01L10 2z" />
                                </svg>
                                Cliente fiel: <span class="text-4xl text-amber-600">{{ $compras }}</span><span
                                    class="text-2xl">/10</span>
                                @if ($premioListo)
                                    <span
                                        class="ml-4 bg-green-600 text-white px-5 py-2 rounded-full font-bold animate-pulse shadow-lg">
                                        Premio listo
                                    </span>
                                @else
                    ...
                    @if ($metodo_pago === 'efectivo')
                        <div class="space-y-6">
                            ...
                            <div
                                class="@if ($cambio >= 0) bg-gradient-to-br from-green-600 to-green-700 @else bg-red-600 @endif text-white p-6 rounded-3xl text-center shadow-2xl">
                    ...
                    @else
                        ...
                        <div class="mt-6 space-y-4">
                            ...
                            <div
                                class="flex flex-col items-center justify-center bg-white rounded-3xl border border-gray-200 p-6">
                                @if ($total > 0)
                                    <div class="bg-white p-4 rounded-2xl shadow-md">
                                        {!! QrCode::size(220)->margin(1)->generate($texto_qr ?: 'PAGO LIBRERIA JAYRITA') !!}
                                    </div>
                                    <p class="mt-3 text-lg font-black text-gray-800">
                                        Monto a cancelar: <span class="text-[#3483FA]">Bs
                    ...
                    <button wire:click="confirmarVenta"
                        @if ($metodo_pago === 'efectivo' && $cambio < 0) disabled
                            ...
                            @else
                            class="w-full py-8 bg-[#3483FA] hover:bg-blue-700 text-white rounded-3xl font-black text-4xl shadow-2xl transform hover:scale-105 transition" @endif>
            ...
            @if ($mostrarModalCliente)
                <div
                    class="fixed inset-0 bg-black bg-opacity-60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div class="bg-white rounded-3xl shadow-3xl max-w-2xl w-full max-h-screen overflow-y-auto">
                        ...
                        <div class="px-8 pb-8 space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div><input type="text" wire:model="nuevoCliente.nombre"
                                        placeholder="Nombre completo *"
                                        class="w-full px-6 py-4 rounded-xl border-2 border-gray-300 focus:border-[#3483FA] focus:ring-4 focus:ring-[#3483FA]/20 text-lg">
                                </div>
                                <div><input type="text" wire:model="nuevoCliente.ci" placeholder="CI / NIT"
                                        class="w-full px-6 py-4 rounded-xl border-2 border-gray-300 focus:border-[#3483FA] focus:ring-4 focus:ring-[#3483FA]/20 text-lg">
                                </div>
                                <div><input type="text" wire:model="nuevoCliente.telefono" placeholder="Teléfono"
                                        class="w-full px-6 py-4 rounded-xl border-2 border-gray-300 focus:border-[#3483FA] focus:ring-4 focus:ring-[#3483FA]/20 text-lg">
                                </div>
                                <div><input type="email" wire:model="nuevoCliente.correo"
                                        placeholder="Correo (opcional)"
                                        class="w-full px-6 py-4 rounded-xl border-2 border-gray-300 focus:border-[#3483FA] focus:ring-4 focus:ring-[#3483FA]/20 text-lg">
                                </div>
                                <div class="md:col-span-2"><input type="text" wire:model="nuevoCliente.direccion"
                            ...
                            <div class="flex gap-4 pt-6">
                                <button wire:click="$set('mostrarModalCliente', false)"
                                    class="flex-1 py-5 rounded-xl bg-gray-200 hover:bg-gray-300 font-bold text-xl transition">Cancelar</button>
                                <button wire:click="crearCliente"
                                    class="flex-1 py-5 rounded-xl bg-[#3483FA] hover:bg-blue-700 text-white font-black text-xl shadow-lg transition">Crear
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.addEventListener('confirmar-venta', e => {
            const d = e.detail;
            let lista =
                '<div style="text-align:left; max-height:220px; overflow-y:auto; font-size:13px;">';
            d.productos.forEach(p => {
                lista +=
                    `• <strong>${p.nombre}</strong> ×${p.cantidad} → Bs ${parseFloat(p.subtotal).toFixed(2)}<br>`;
            });
            lista += '</div>';
            Swal.fire({
        ...
        window.addEventListener('premio-cliente-fiel', event => {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) @this.call('entregarPremioClienteFiel', data
                .cliente_id);
            });
        });
        ...
        document.addEventListener('keydown', e => {
            ...
            if (e.key === 'F4') {
                e.preventDefault();
                @this.call('confirmarVenta');
            }
            if (e.key === 'F9') {
                e.preventDefault();
                @this.call('limpiarCarrito');
            }
            if (e.key === 'F2') {
                e.preventDefault();
