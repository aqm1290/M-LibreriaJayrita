<div class="min-h-screen bg-gradient-to-br from-yellow-100 via-yellow-50 to-orange-100 py-12 px-6">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-yellow-200 p-5 md:p-6">
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                <input type="text" wire:model.live.debounce.500ms="search" placeholder="Buscar marca..."
            ...
            <div class="hidden lg:block overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-yellow-50">
                        @forelse($marcas as $m)
                            <tr
                                class="{{ $m->activo ? 'hover:bg-yellow-50/70' : 'bg-rose-50/30 hover:bg-rose-50/60' }} transition-colors">
                                <td class="px-6 py-5 align-top">
                                    @if ($m->url_imagen)
                                        <img src="{{ asset('storage/' . $m->url_imagen) }}" alt="{{ $m->nombre }}"
                                            class="w-14 h-14 object-contain rounded-xl shadow-md border border-yellow-200 bg-white">
                                    @else
                                        <div
                                            class="w-14 h-14 bg-yellow-50 border border-dashed border-yellow-300 rounded-xl flex items-center justify-center">
                                            <span class="text-[0.65rem] font-semibold text-yellow-700">SIN LOGO</span>
                                        </div>
                                    @endif
                                </td>
                                <td class="px-6 py-5 align-top">
                                ...
                                <td class="px-6 py-5 align-top">
                                    <span
                                        class="inline-flex items-center px-4 py-1.5 rounded-full bg-yellow-100
                                               text-yellow-800 font-semibold text-xs">
                    ...
                    <div class="{{ $m->activo ? 'bg-white' : 'bg-rose-50/40' }} p-5">
                        <div class="flex items-start gap-4">
                            <div>
                                @if ($m->url_imagen)
                                    <img src="{{ asset('storage/' . $m->url_imagen) }}" alt="{{ $m->nombre }}"
                                        class="w-16 h-16 object-contain rounded-xl shadow-md border border-yellow-200 bg-white">
                                @else
                                    <div
                                        class="w-16 h-16 bg-yellow-50 border border-dashed border-yellow-300 rounded-xl flex items-center justify-center">
                                        <span class="text-[0.7rem] font-semibold text-yellow-700">LOGO</span>
                                    </div>
                                @endif
                            </div>
    ...
    @if ($modal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div class="w-full max-w-3xl rounded-3xl bg-white/95 shadow-2xl border border-yellow-200">
                <div
                ...
                <div class="px-8 py-7 space-y-7">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" wire:model="nombre"
                                class="w-full px-4 py-2.5 rounded-2xl border border-yellow-300 bg-white/90 text-sm
                        ...
                        <div>
                            ...
                            <input type="file" wire:model="imagen" accept="image/*"
                                class="block w-full text-xs text-slate-700
                                       ...
                                       file:bg-yellow-500 file:text-white hover:file:bg-orange-600
                                       cursor-pointer">

                            <div class="mt-4 flex justify-start">
                                @if ($imagen)
                                    <img src="{{ $imagen->temporaryUrl() }}"
                                        class="w-24 h-24 object-contain rounded-xl shadow-md border border-yellow-200 bg-white">
                                @elseif($url_imagen)
                                    <img src="{{ asset('storage/' . $url_imagen) }}"
                                        class="w-24 h-24 object-contain rounded-xl shadow-md border border-yellow-200 bg-white">
                                @else
                                    <div
                                        class="w-24 h-24 bg-yellow-50 border border-dashed border-yellow-300 rounded-xl flex items-center justify-center">
                                        <span class="text-[0.7rem] font-semibold text-yellow-700">SIN LOGO</span>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div>
                        ...
                        <textarea wire:model="descripcion" rows="3"
                            class="w-full px-4 py-3 rounded-2xl border border-yellow-300 bg-white/90 text-sm
    ...
    @if ($confirmDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div
                class="w-full max-w-md rounded-3xl bg-white/95 shadow-2xl border border-yellow-200 px-8 py-7 text-center">
                <div
                    class="w-20 h-20 mx-auto mb-5 rounded-2xl flex items-center justify-center
                    {{ $this->marca?->activo ? 'bg-rose-100' : 'bg-emerald-100' }}">
                    <svg class="w-10 h-10 {{ $this->marca?->activo ? 'text-rose-600' : 'text-emerald-600' }}"
