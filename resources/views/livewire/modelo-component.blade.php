<div class="min-h-screen bg-gray-50 py-12 px-6">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-10">
            <div>
                <h1 class="text-5xl font-black text-gray-900">Gestión de Modelos</h1>
                <p class="text-xl text-gray-600 mt-2">Administra los modelos por marca</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
            <table class="w-full">
                <thead class="bg-gray-900 text-white">
                    <tr>
                        <th class="px-8 py-6 text-left font-bold text-lg">Modelo</th>
                        <th class="px-8 py-6 text-left font-bold text-lg">Marca</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($modelos as $m)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-6 font-bold text-gray-900 text-lg">{{ $m->nombre }}</td>
                            <td class="px-8 py-6">
                                <span class="px-1 py-1 bg-purple-100 text-purple-800 font-bold rounded-full">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-20 text-gray-500 text-2xl font-bold">
        ...
        <div class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-6">
            <div class="bg-white rounded-2xl shadow-3xl w-full max-w-2xl">
                ...
                <div class="p-10 space-y-8">
                    <div>
                        <label class="block text-lg font-bold text-gray-800 mb-3">Marca *</label>
                        ...
                        @error('marca_id') <span class="text-red-600 font-bold mt-2 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-lg font-bold text-gray-800 mb-3">Nombre del modelo *</label>
    ...
    @if($confirmDelete)
        <div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-6">
            <div class="bg-white rounded-2xl shadow-3xl max-w-md w-full p-10 text-center">
                ...
                <h3 class="text-3xl font-black text-gray-900 mb-4">¿Eliminar modelo?</h3>
                <p class="text-lg text-gray-600 mb-8">
                    Esta acción no se puede deshacer.<br>
                    @if($modeloId && ($m = \App\Models\Modelo::find($modeloId)) && $m->productos()->count() > 0)
                        <strong class="text-red-600">¡Hay {{ $m->productos()->count() }} productos con este modelo!</strong>
                    @endif
                </p>
